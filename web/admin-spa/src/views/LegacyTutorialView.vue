<template>
  <div :class="embeddedFlag ? '' : 'card p-3 sm:p-6'">
    <div v-if="!hideHeaderFlag" class="mb-4 sm:mb-8">
      <h3
    ...
    <!-- 系统选择标签 -->
    <div v-if="!hideSystemTabsFlag" class="mb-4 sm:mb-8">
    ...
    <div v-if="activeTutorialSystem === 'windows'" class="tutorial-content">
      ...
      <div class="mb-4 sm:mb-10 sm:mb-6">
        ...
        <p class="mb-4 text-sm text-gray-600 dark:text-gray-400 sm:mb-4 sm:mb-6 sm:text-base">
          建议先安装 Node.js 18+ 环境，后续 CLI/脚本会用到。
        </p>
      ...
      <!-- 第二步：安装 Claude Code -->
      <div v-if="isClaude" class="mb-4 sm:mb-10 sm:mb-6">
        <h4
          class="mb-3 flex items-center text-lg font-semibold text-gray-800 dark:text-gray-300 sm:mb-4 sm:text-xl"
        >
          <span
      ...
      <!-- 第三步：设置环境变量 -->
      <div v-if="isClaude || isGemini || isCodex" class="mb-6 sm:mb-10">
        <h4
          class="mb-3 flex items-center text-lg font-semibold text-gray-800 dark:text-gray-300 sm:mb-4 sm:text-xl"
        >
          <span
          ...
          <div class="space-y-4">
            ...
            >
              ...
              <p class="mt-2 text-xs text-blue-700 dark:text-blue-300">
                提示：Claude Code 实际请求端点为
                <code class="rounded bg-gray-100 px-1 dark:bg-gray-800"
                  >{{ currentBaseUrl }}/v1/messages</code
                >。
              </p>
      ...
      <div v-if="isDroid" class="mt-8">
        ...
        <p class="mb-3 text-sm text-gray-700 dark:text-gray-300 sm:mb-4 sm:text-base">
          ...
          >
          保存自定义模型；你也可以在资源管理器地址栏输入
          <code class="rounded bg-gray-100 px-1 dark:bg-gray-800">%USERPROFILE%\.factory</code>
          快速打开配置目录。
        </p>
        <div
          class="rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-500/40 dark:bg-blue-950/30 sm:p-4"
      ...
      <!-- 第四步：开始使用 -->
      <div v-if="isClaude" class="mb-6 sm:mb-8">
        <h4
          class="mb-3 flex items-center text-lg font-semibold text-gray-800 dark:text-gray-300 sm:mb-4 sm:text-xl"
        >
          <span
    ...
    <div v-else-if="activeTutorialSystem === 'macos'" class="tutorial-content">
      ...
      <!-- 第二步：安装 Claude Code -->
      <div v-if="isClaude" class="mb-6 sm:mb-10">
        <h4
          class="mb-3 flex items-center text-lg font-semibold text-gray-800 dark:text-gray-300 sm:mb-4 sm:text-xl"
        >
          <span
...
// 当前系统选择
const activeTutorialSystem = ref(normalizeSystem(props.initialSystem))

watch(
  () => props.initialSystem,
  (value) => {
    activeTutorialSystem.value = normalizeSystem(value)
  }
)

const hideSystemTabsFlag = computed(() => Boolean(props.hideSystemTabs))
const hideHeaderFlag = computed(() => Boolean(props.hideHeader))
const embeddedFlag = computed(() => Boolean(props.embedded))

const normalizeModel = (value) => {
  const key = String(value || '')
    .trim()
    .toLowerCase()
  if (key === 'claude' || key === 'codex' || key === 'gemini' || key === 'droid') {
    return key
  }
  return 'claude'
}

const activeTargetModel = computed(() => normalizeModel(props.targetModel))

const isClaude = computed(() => activeTargetModel.value === 'claude')
const isCodex = computed(() => activeTargetModel.value === 'codex')
const isGemini = computed(() => activeTargetModel.value === 'gemini')
const isDroid = computed(() => activeTargetModel.value === 'droid')

const envSetupStepNumber = computed(() => (isClaude.value ? 3 : 2))

const tutorialTitle = computed(() => {
  switch (activeTargetModel.value) {
    case 'codex':
      return 'Codex 使用教程'
    case 'gemini':
      return 'Gemini 使用教程'
...
const tutorialDescription = computed(() => {
  switch (activeTargetModel.value) {
    case 'codex':
      return '配置 Codex 连接到你的中转服务，并完成鉴权配置。'
    case 'gemini':
      return '配置 Gemini CLI 连接到你的中转服务，并完成环境变量设置。'
    case 'droid':
      return '配置 Droid CLI（Factory.ai）连接到你的中转服务，并完成自定义模型配置。'
    case 'claude':
    default:
      return '跟着这个教程，你可以轻松在自己的电脑上安装并使用 Claude Code。'
  }
})
