        ...
        <div class="user-menu-container relative">
          ...
          >
            ...
            <div class="border-b border-gray-100 px-4 py-3 dark:border-gray-700">
              ...
              <div v-if="versionInfo.hasUpdate" class="mt-2">
                <div class="mb-2 flex items-center justify-between text-sm">
                  <span class="font-medium text-green-600 dark:text-green-400">
                    <i class="fas fa-arrow-up mr-1" />{{ t('header.newVersionAvailable') }}
                  </span>
              ...
              >
                <i class="fas fa-spinner fa-spin mr-1" />{{ t('header.checkingUpdates') }}
              </div>
              <div v-else class="mt-2 text-center">
                ...
                <transition mode="out-in" name="fade">
                  ...
                  >
                    <i class="fas fa-sync-alt mr-1" />{{ t('header.checkUpdates') }}
                  </button>
                </transition>
              </div>
            </div>
            ...
            >
              ...
              <span>{{ t('header.changeAccount') }}</span>
            </button>
      ...
      >
        <div>
          ...
          <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
            {{ t('header.currentUsernameHelp') }}
          </p>
        </div>

        <div>
          <label class="mb-3 block text-sm font-semibold text-gray-700 dark:text-gray-300">{{
            t('header.newUsername')
          }}</label>
          <input
            v-model="changePasswordForm.newUsername"
            class="form-input w-full"
            :placeholder="t('header.newUsernamePlaceholder')"
            type="text"
          />
          <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
            {{ t('header.newUsernameHelp') }}
          </p>
        </div>
        ...
        <div class="flex gap-3 pt-4">
          ...
          >
            ...
            {{ changePasswordLoading ? t('common.saving') : t('header.saveChanges') }}
          </button>
        </div>
      </form>
    </div>
...
const changePassword = async () => {
  ...
  if (changePasswordForm.newPassword.length < 8) {
    showToast(t('messages.passwordMinLength'), 'error')
    return
  }
  ...
  try {
    ...
    if (data.success) {
      const message = changePasswordForm.newUsername
        ? t('messages.accountUpdatedRelogin')
        : t('messages.passwordUpdatedRelogin')
      ...
      showToast(data.message || t('messages.updateFailed'), 'error')
    }
  } catch (error) {
    showToast(t('messages.passwordUpdateFailed'), 'error')
  } finally {
