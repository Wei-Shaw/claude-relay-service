  ...
  <div class="space-y-1">
    ...
    <div v-if="showProgress" class="space-y-0.5">
      <div v-if="hasRequestLimit" class="space-y-0.5">
        <div class="flex items-center justify-between text-xs">
          <span class="text-gray-400">{{ t('apiKeyLimits.requests') }}</span>
      ...
      <div v-if="hasCostLimit" class="space-y-0.5">
        <div class="flex items-center justify-between text-xs">
          <span class="text-gray-400">{{ t('apiKeyLimits.cost') }}</span>
          <span class="text-gray-600">
    ...
    <div v-if="windowState === 'active' && showTooltip" class="text-xs text-gray-500">
      ...
      <span v-if="remainingSeconds < 60">{{ t('apiKeyLimits.resetSoon') }}</span>
      <span v-else-if="remainingSeconds < 300">{{
        t('apiKeyLimits.resetInMinutes', { count: Math.ceil(remainingSeconds / 60) })
      }}</span>
      <span v-else>{{
        t('apiKeyLimits.resetAfter', { duration: formatDetailedTime(remainingSeconds) })
      }}</span>
    </div>
  </div>
</template>
...
// 计算属性
const displayLabel = computed(() => props.label || t('apiKeyLimits.windowLimit'))
...
const formatDetailedTime = (seconds) => {
  ...
  if (hours > 0) {
    return t('apiKeyLimits.hoursMinutes', { hours, minutes })
  } else {
    return t('apiKeyLimits.minutes', { count: minutes })
  }
}
