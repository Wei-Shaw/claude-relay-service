...
项目包含 GitHub Actions 工作流 `.github/workflows/dev-private-deploy.yml`，支持自动构建并部署到私有仓库和服务器。

- **触发方式**: 推送任意分支 (`git push`)
- **构建产物**: Docker 镜像推送到私有仓库
- **部署流程**: SSH 登录服务器 -> 拉取新镜像 -> 设置 `DOCKER_IMAGE_NAME` -> 重启服务
- **环境变量**: 服务器端需配置 `.env` (JWT_SECRET, ENCRYPTION_KEY) 且不被 Git 管理
...
# 执行主函数
main "$@"
  报错 报JWT_SECRET没设置    以上是原生成docker-compose.yml的逻辑
- !/bin/bash
# ================================================
# Claude Relay Service Docker Compose 生成脚本
# 功能：生成包含安全密钥的 docker-compose.yml 文件
