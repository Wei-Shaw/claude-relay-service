...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    // CLI 支持: codex_vscode, codex_cli_rs, codex_exec (非交互式/脚本模式)
    // App 支持: Codex Desktop
    ...
    const isCodexApp = codexAppPattern.test(userAgent)
    const isCodexClient = isCodexCLI || isCodexApp

    // 如果不是 Codex 客户端请求，则进行适配
    if (!isCodexClient) {
      // 移除不需要的请求体字段
      ...
      logger.info('📝 Non-Codex client request detected, applying Codex adaptation')
    } else {
      const clientLabel = isCodexApp ? 'Codex App' : 'Codex CLI'
      logger.info(`✅ ${clientLabel} request detected, forwarding as-is`)
    }
    ...
    if (isCodexClient) {
      const passthroughKeys = ['originator', 'user-agent', 'x-oai-web-search-eligible']
