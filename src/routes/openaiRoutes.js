...
async function resolveOpenAIAccountSelection(selection) {
  ...
  const { accountId } = selection
  const accountType = selection.accountType || 'openai'
  ...
  if (accountType === 'openai-responses') {
    ...
    if (!account || !account.apiKey) {
      const error = new Error(`OpenAI-Responses account ${accountId} has no valid apiKey`)
    ...
    return { accessToken, accountId, accountType, proxy, account }
  }

  account = await openaiAccountService.getAccount(accountId)
  if (!account || !account.accessToken) {
    const error = new Error(`OpenAI account ${accountId} has no valid accessToken`)
    error.statusCode = 403
    throw error
  }
  ...
  logger.info(`Selected OpenAI account: ${account.name} (${accountId})`)

  return { accessToken, accountId, accountType, proxy, account }
}

/**
 * Execute OpenAI request with failover support
 * @param {Object} params
 ...
 */
async function executeOpenAIWithFailover({
  req,
  res,
  apiKeyData,
  sessionHash,
...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    await executeOpenAIWithFailover({
      req,
      res,
      apiKeyData,
      sessionHash,
      requestedModel,
      executeWithAccount: async (selection) => {
        let upstream = null
        let accessToken,
          proxy,
          account = null
        ...
        ;({ accessToken, accountId, accountType, proxy, account } =
          await resolveOpenAIAccountSelection(selection))
...
module.exports.executeOpenAIWithFailover = executeOpenAIWithFailover
