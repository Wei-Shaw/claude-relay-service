...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    // 客户端断开时回收上游流。原先挂 req 'close'/'aborted'：前者在请求体读完时早已触发完毕
    // （此处在 await 之后注册, 恒不触发=死监听），故断开时上游流从不回收。判据收口在 utils/clientDisconnect。
    // 上游请求的中止由上方 upstreamAbort 负责（signal 已在发请求前就绪），此处只补流对象的回收
    onClientDisconnect(
      res,
      () => {
        try {
