...
async function handleMessagesRequest(req, res) {
  try {
    ...
    if (isStream) {
      ...
      await executeWithFailover({
        ...
        selectAccount: async (excludeAccountIds) => {
          try {
            return await unifiedClaudeScheduler.selectAccountForApiKey(
              req.apiKey,
              sessionHash,
              requestedModel,
            ...
            await claudeRelayService.relayStreamRequestWithUsageCapture(
              ...
              (usageData) => {
                ...
                if (
                  ...
                  logger.api(
                    `📊 Stream usage recorded (real) - Model: ${model}, Input: ${inputTokens}, Output: ${outputTokens}, Cache Create: ${cacheCreateTokens}, Cache Read: ${cacheReadTokens}, Total: ${
            ...
            await ccrRelayService.relayStreamRequestWithUsageCapture(
              ...
              (usageData) => {
                ...
                if (
                  ...
                  usageDataCaptured = true
                  logger.api(
                    `📊 CCR stream usage recorded (real) - Model: ${model}, Input: ${inputTokens}, Output: ${outputTokens}, Cache Create: ${cacheCreateTokens}, Cache Read: ${cacheReadTokens}, Total: ${
                      inputTokens + outputTokens + cacheCreateTokens + cacheReadTokens
                    } tokens`
                  )
                } else {
    ...
    if (handledError.code === 'CLAUDE_DEDICATED_RATE_LIMITED' && res.headersSent) {
      return undefined
    }
