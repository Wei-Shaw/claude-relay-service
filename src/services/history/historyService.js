...
const AUTO_GENERATED_USER_PATTERNS = [
  ...
  /packages?\s+are\s+looking\s+for\s+funding/i,
  /npm\s+(?:warn|notice|err|info)/i,
  /npm\s+warn\s+exec/i,
  /run\s+`?npm\s+[^`]+`?/i,
  /npx\s+chrome-devtools-mcp@/i,
  /file\s+created\s+successfully\s+at:/i
]
...
const matchesAutoGeneratedContent = (content) => {
  ...
  if (lines.some((line) => AUTO_GENERATED_USER_PATTERNS.some((pattern) => pattern.test(line)))) {
    return true
  }

  const cliOptionLines = lines.filter((line) => /--[a-z0-9][\w-]*\s{2,}/i.test(line)).length
  if (cliOptionLines >= 2) {
    return true
  }
  ...
  if (npmIndicators.some((pattern) => pattern.test(trimmed) || lines.some((line) => pattern.test(line)))) {
    return true
  }
